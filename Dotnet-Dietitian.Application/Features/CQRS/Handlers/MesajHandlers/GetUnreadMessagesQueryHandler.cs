...
{
    public class GetUnreadMessagesQueryHandler : IRequestHandler<GetUnreadMessagesQuery, List<GetMesajQueryResult>>
    {
        ...
        public async Task<List<GetMesajQueryResult>> Handle(GetUnreadMessagesQuery request, CancellationToken cancellationToken)
        {
            var mesajlar = await _mesajRepository.GetUnreadMessagesAsync(request.UserId, request.UserType);
            ...
            return result.OrderByDescending(m => m.GonderimZamani).ToList();
        }
    }
} 